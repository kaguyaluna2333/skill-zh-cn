...
跨宿主的 skill / 命令 / 子智能体说明汉化工具。一个宿主无关内核 + 通用 CLI，把 **Claude Code / zcode / opencode / codex** 等工具里显示给用户的 `description` 翻译成简体中文。

> 装在谁身上，就让谁的 agent 翻——不依赖任何外部 CLI，也不需要 API key。
